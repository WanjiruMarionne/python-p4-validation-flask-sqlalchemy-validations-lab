...
class Author(db.Model):
    ...
    def validate_name(self, key, name):
        ...
        if existing_author and existing_author.id != self.id:
            raise ValueError(f"Author called {name} already exists.")
...
class Post(db.Model):
    ...
    def validate_title(self, key, title):
        ...
        if not any(keyword in title for keyword in clickbaity_keywords):
            raise ValueError("Post title must contain either of these words: 'Won't Believe', 'Secret', 'Top', 'Guess'")
        return title
